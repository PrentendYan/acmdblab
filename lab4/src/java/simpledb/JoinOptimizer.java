...
// Iteratively build up plans for increasing sizes of join subsets
        for (int subsetSize = 1; subsetSize <= joins.size(); ++subsetSize) {
            Set<Set<LogicalJoinNode>> subsets = enumerateSubsets(joins, subsetSize);
            for (Set<LogicalJoinNode> currentSubset : subsets) {
                bestPlan = null; // Reset bestPlan for each subset
                for (LogicalJoinNode joinNode : currentSubset) {
                    double maxCost = bestPlan == null ? Double.MAX_VALUE : bestPlan.cost;
                    CostCard currentPlan = computeCostAndCardOfSubplan(
                            stats, filterSelectivities, joinNode, currentSubset, maxCost, planCache
                    );
                    if (currentPlan != null && (bestPlan == null || currentPlan.cost < bestPlan.cost)) {
                        bestPlan = currentPlan; // Update bestPlan if currentPlan is better
                    }
                }
                // Store the best plan found for this subset in the cache
                if (bestPlan != null) {
                    planCache.addPlan(currentSubset, bestPlan.cost, bestPlan.card, bestPlan.plan);
                }
            }
        }
        // Retrieve the best order for the full set of joins
