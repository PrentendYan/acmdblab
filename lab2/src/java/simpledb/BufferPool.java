...
public class BufferPool {
    ...
    private void LRUUpdate(PageId pid) {
        LRUCount.put(pid, 2 * maxNumPages);
    ...
    public  Page getPage(TransactionId tid, PageId pid, Permissions perm)
        ...
        if (!pageMap.containsKey(pid)) {
            while (pageMap.size() >= maxNumPages) {
                evictPage();
            }
            requestedPage = Database.getCatalog().getDatabaseFile(pid.getTableId()).readPage(pid);
            pageMap.put(pid, requestedPage);
        } else {
            requestedPage = pageMap.get(pid);
        }
        LRUUpdate(pid);
    ...
    private void handleDirtyPages(TransactionId tid, List<Page> dirtypages) throws DbException {
        for (Page page: dirtypages) {
            ...
            while (!pageMap.containsKey(pid) && pageMap.size() >= maxNumPages) {
                evictPage();
            }
    ...
    */
    ...
    private synchronized  void evictPage() throws DbException {
        // some code goes here
        // not necessary for lab1
        Map.Entry<PageId, Integer> leastRecentlyUsedEntry = null;
        for (Map.Entry<PageId, Integer> entry : LRUCount.entrySet()) {
            if (leastRecentlyUsedEntry == null || entry.getValue() < leastRecentlyUsedEntry.getValue()) {
                leastRecentlyUsedEntry = entry;
            }
        }

        if (leastRecentlyUsedEntry != null) {
            PageId pageToEvict = leastRecentlyUsedEntry.getKey();
            ...
            pageMap.remove(pageToEvict);
            LRUCount.remove(pageToEvict);
        }
    }
