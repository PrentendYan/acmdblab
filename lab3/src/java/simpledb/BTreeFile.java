...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// Iterate through entries to find the correct child to visit
		for (BTreeInternalPageIterator iterator = (BTreeInternalPageIterator) page.iterator(); iterator.hasNext(); ) {
			entry = iterator.next();
			// Determine whether to go left based on the comparison with the provided field
		...
		// If no appropriate left child is found, go to the right child of the last entry
		if (toVisit == null && entry != null) {
			toVisit = entry.getRightChild();
		}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		ArrayList<Tuple> tuplesToMove = new ArrayList<>();

		// Collect tuples to move to the new leaf page
		Iterator<Tuple> reverseIterator = page.reverseIterator();
		for (int i = 0; i < numTuples / 2; i++) {
			tuplesToMove.add(reverseIterator.next());
		}

		// Move the tuples and update the original and new leaf pages
		Tuple middleTuple = reverseIterator.next();  // This tuple's key will be promoted to the parent page
		...
		if (rightSiblingId != null) {
			...
			rightSiblingPage.setLeftSiblingId(newLeaf.getId());
		}

		// Create a new entry for the parent page
		Field middleKey = middleTuple.getField(0);  // Assuming the field index for the key is 0
		BTreeEntry newParentEntry = new BTreeEntry(middleKey, page.getId(), newLeaf.getId());
		addParentEntryForSplitPages(tid, dirtypages, page, newLeaf, newParentEntry);

		// Return the appropriate page based on the provided field
		return middleKey.compare(Op.GREATER_THAN_OR_EQ, field) ? page : newLeaf;
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// Initialize iterator for tuples on the right page
		Iterator<Tuple> iterator = rightPage.iterator();
		LinkedList<Tuple> tuplesToMove = new LinkedList<>();

	// Collect all tuples from the right page
	...
	// Update sibling pointers after moving tuples
		BTreePageId rightSiblingId = rightPage.getRightSiblingId();
		leftPage.setRightSiblingId(rightSiblingId);

	// If the right page has a right sibling, update its left sibling pointer
		if (rightSiblingId != null) {
			BTreeLeafPage rightSibling = (BTreeLeafPage) getPage(tid, dirtypages, rightSiblingId, Permissions.READ_WRITE);
			rightSibling.setLeftSiblingId(leftPage.getId());
		}

	// Mark the right page as empty after moving all tuples
		setEmptyPage(tid, dirtypages, rightPage.getId().pageNumber());

	// Update the parent after removing the right page
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// Delete the entry in the parent corresponding to the two pages that are merging -
		// deleteParentEntry() will be useful here
		// Initialize the iterator for entries on the right page
		Iterator<BTreeEntry> iterator = rightPage.iterator();
		LinkedList<BTreeEntry> entriesToMove = new LinkedList<>();

	// Collect all entries from the right page
		while (iterator.hasNext()) {
			entriesToMove.add(iterator.next());
		}

	// Insert a new entry into the left page using the parent's key and the appropriate children
		BTreeEntry firstEntryToMove = entriesToMove.getFirst();
		BTreeEntry newParentEntry = new BTreeEntry(
				parentEntry.getKey(),
				leftPage.reverseIterator().next().getRightChild(),
				firstEntryToMove.getLeftChild()
		);
		leftPage.insertEntry(newParentEntry);

	// Move all entries from the right page to the left page
		for (BTreeEntry entry : entriesToMove) {
			rightPage.deleteKeyAndLeftChild(entry);
			leftPage.insertEntry(entry);
		}

	// Update parent pointers for both pages after modifying their entries
		updateParentPointers(tid, dirtypages, leftPage);
		updateParentPointers(tid, dirtypages, rightPage);

	// Mark the right page as empty after moving all entries
		setEmptyPage(tid, dirtypages, rightPage.getId().pageNumber());

	// Update the parent structure after removing the right page
		deleteParentEntry(tid, dirtypages, leftPage, parent, parentEntry);
	}
