    ...
    public double estimateSelectivity(Predicate.Op op, int v) {
        ...
        switch (op) {
            case EQUALS:
                ...
                numTuples = histogram[bucketIndex] / (double) width;
                break;
            case NOT_EQUALS:
            ...
            case LESS_THAN:
                ...
                for (int i = 0; i < bucketIndex; i++) {
                    numTuples += histogram[i];
                }
                fraction = (v - leftBound(bucketIndex)) / (double) width;
                numTuples += histogram[bucketIndex] * fraction;
                break;
            ...
            case GREATER_THAN:
                if (v < min) return 1.0;
                if (v >= max) return 0.0;
                for (int i = bucketIndex + 1; i < bucketNum; i++) {
                    numTuples += histogram[i];
                }
                numTuples += histogram[bucketIndex] * (rightBound(bucketIndex) - v - 1) / (double) width;
                break;
